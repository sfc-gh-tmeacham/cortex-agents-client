...
Field semantics:
- `analyst_sql`: Maps `tool_use_id` to generated SQL queries, rendered inside the tool status expander.
...
6. On `ThinkingDeltaEvent` (if `show_thinking=True`): stream reasoning text into a compact `st.expander("Reasoning", icon=":material/psychology:", type="compact")`.
7. On `ThinkingEvent`: record final thinking text on `stored.thinking`.
8. On `ToolUseEvent`:
   ...
   - If `show_tool_status=True`: open `container.status(f":material/build: Using {event.name}...", expanded=False, type="step")`. If SQL is present in `stored.analyst_sql`, display it as a code block.
9. On `ToolResultStatusEvent`: update tool status label.
10. On `ToolResultEvent`: close status context with complete state (`:material/check_circle:` or `:material/verified:`) or error state (`:material/error:`). Render any text items from `event.content` via markdown and record in `stored.tool_result_text`.
...
19. Post-stream finalization:
    - If the stream ended before a tool result arrived, update lingering tool status contexts to "Tool interrupted" (`state="error"`).
...
2. Tool execution results: one pass per tool execution in `msg.tool_executions`, rendering the status expander with SQL (if present) followed by tool result text (if present).
...
- `key_prefix`: Assigns deterministic keys to child widgets, creating stable `.st-key-*` CSS classes in the DOM (`.st-key-{prefix}-thinking`, `.st-key-{prefix}-sources`, `.st-key-{prefix}-table-{i}`, `.st-key-{prefix}-chart-{i}`).
